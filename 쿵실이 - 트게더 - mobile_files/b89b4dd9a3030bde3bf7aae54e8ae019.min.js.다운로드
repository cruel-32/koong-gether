$(document).ready(function(){var $header=$('header');var didScroll;var lastScrollTop=0;var delta=5;var navbarHeight=$header.outerHeight();$(window).scroll(function(event){didScroll=!0});setInterval(function(){if(didScroll){hasScrolled();didScroll=!1}},250);function hasScrolled(){var st=$(this).scrollTop();if(Math.abs(lastScrollTop-st)<=delta)
return;if(st>lastScrollTop&&st>navbarHeight){$header.removeClass('nav-down').addClass('nav-up')}else{if(st+$(window).height()<$(document).height()){$header.removeClass('nav-up').addClass('nav-down')}}
lastScrollTop=st}
$.TGD=$.TGD||{};$.TGD.latest=function(){var cookie_id='rclist';var list=[];var max_length=10;return{init:function(noAdd){$('#latestDropdownMenu').empty()
...
for(var i=0;i<r.favlist.length;++i){$('#favoriteDropdownMenu').append('<li><a data-id="'+r.favlist[i].favorite_id+'" href="/'+r.favlist[i].alias+'"><span class="name">'+r.favlist[i].name+'</span> <span class="delrow"><i class="fa fa-remove"></i></span></a></li>')}})};if($('#notification-badge').length>0){$.TGD.loadNotify()}
...
e.stopPropagation()
$.post('/member/config/fav',{remove:[$(this).parents('a').attr('data-id')]},function(r){$.TGD.loadNotify()})});var $window=$(window);$window.scroll(function(){$('#movetotop')[$window.scrollTop()>100?'fadeIn':'fadeOut']()})
$('#movetotop').click(function(e){e.preventDefault()
$(window).scrollTop(0)})
var clickEventType=((document.ontouchstart!==null)?'click':'touchstart');var $nav=$('nav');$('#header-bar-link').click(function(e){e.stopPropagation();e.preventDefault();$('nav,#nav-overlay').removeClass('hide')});$('#header-close-bar-link').click(function(e){e.preventDefault();if($nav.hasClass('hide')===!1){$('nav,#nav-overlay').addClass('hide')}})
$('#header-search-link').click(function(e){e.preventDefault();$('#header-area').addClass('hide');$('#header-search-bar').removeClass('hide');$('#header-search-input').focus()})
$('#header-close-search-btn').click(function(e){e.preventDefault();$('#header-area').removeClass('hide');$('#header-search-bar').addClass('hide')})
$(document).on(clickEventType,'body',function(evt){if(evt.target.id=="nav")
return;if($(evt.target).closest('#nav').length)
return;if($nav.hasClass('hide')===!1){$('nav,#nav-overlay').addClass('hide')}})
...
$('a.nav-menu').click(function(e){if(ga)ga('send','event',{eventCategory:'Nav Link',eventAction:'click',eventLabel:$(this).attr('href'),transport:'beacon'})})
...
$('#header-search-form').submit(function(e){if(ga)ga('send','event',{eventCategory:'Search',eventAction:'click',eventLabel:$('#header-search-input').val().trim(),transport:'beacon'})})
...
$('#mobile_menu_footer_banner a').click(function(e){if(ga)ga('send','event',{eventCategory:'Mobile Menu Footer Banner',eventAction:'click',eventLabel:''})})
...
$.TGD.loadNotify()}},'json')});if(Cookies.get('hn')){$('#hide_notice').prop('checked',!0)
...
if($('#article-info').length>0){(adsbygoogle=window.adsbygoogle||[]).push({google_ad_client:"ca-pub-5807968391783575",enable_page_level_ads:!0})}else{$('div.article-list-row:visible').eq(10).after('<div class="article-list-row" style="text-align: center; padding: 5px 0; border-bottom: 1px solid #e2e2e2;"><!-- tgd_board_list_mobile_bottom --><ins class="adsbygoogle" style="display:inline-block;width:320px;height:100px" data-ad-client="ca-pub-5807968391783575" data-ad-slot="6601867647"></ins></div>');(adsbygoogle=window.adsbygoogle||[]).push({})}})